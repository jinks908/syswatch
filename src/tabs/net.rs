...
fn draw_aggregate(f: &mut Frame, area: Rect, app: &App, snap: &Snapshot) {
    ...
    let counters = vec![
        Line::from(vec![
            Span::styled("now   ", Style::default().fg(p::text_muted())),
            Span::styled(
                human_rate(total),
        ...
        Line::from(vec![
            Span::styled("rx    ", Style::default().fg(p::text_muted())),
            Span::styled(human_rate(rx), Style::default().fg(p::status_good())),
        ]),
        Line::from(vec![
            Span::styled("tx    ", Style::default().fg(p::text_muted())),
            Span::styled(human_rate(tx), Style::default().fg(p::brand())),
        ]),
        Line::from(vec![
